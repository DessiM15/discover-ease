...
const navigation: NavItem[] = [
  ...
  { name: "Court Filing", href: "/court-filing", icon: Gavel },
...
// Separate component for expandable nav items to properly use hooks
function ExpandableNavItem({ item, pathname }: { item: NavItem; pathname: string | null }) {
  const isActive = pathname === item.href || pathname?.startsWith(item.href + "/");
  const [isExpanded, setIsExpanded] = useState(pathname?.startsWith(item.href + "/") || false);
    ...
    <div className="flex h-screen w-64 flex-col border-r border-border bg-card">
      ...
      <nav className="flex-1 space-y-1 px-3 py-4 overflow-y-auto">
        {navigation.map((item) => {
          ...
          if (hasChildren) {
            return <ExpandableNavItem key={item.name} item={item} pathname={pathname} />;
          }
